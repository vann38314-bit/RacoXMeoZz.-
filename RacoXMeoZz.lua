...
local v466 = vu32:MakeWindow({
    Title = "Zuda X RacoXMeoZz",
    SubTitle = "Zuda X RacoXMeoZz",
    SaveFolder = "Zuda_X_RacoXMeoZz.lua"
})

-- ========================
-- HIỆU ỨNG CHẠY CHỮ TITLE
-- ========================
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "ZudaTitleGui"
...
TitleLabel.Position = UDim2.new(0, 200, 0, 30) -- chỉnh tọa độ nếu chưa khớp
...
local TextChay = "Zuda X RacoXMeoZz  •  "
local ViTri = 1

task.spawn(function()
    while task.wait(0.12) do
        local HienThi = ""
        for i = 1, #TextChay * 2 do
            local index = ((ViTri + i - 1) % #TextChay) + 1
            HienThi = HienThi .. TextChay:sub(index, index)
        end
        TitleLabel.Text = HienThi
        ViTri = ViTri + 1
    end
end)
